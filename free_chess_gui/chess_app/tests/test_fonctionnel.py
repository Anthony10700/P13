...
class UrlchessappTests(TestCase):
    """
    Class test of url of app chessap
    ...
    def test_history(self):
        """
        test history game with selenium
        """
        info = {"inputemail": "anthony.thillerot@laposte.nettt",
                "inputPassword1": "azerty",
                }
        ...
        self.assertEqual(self.browser.title, "Account")
        time.sleep(2)
        self.browser.get('http://127.0.0.1:8000/chess_app/play_vs_komodo.html')
        time.sleep(2)
        grid = self.browser.find_element_by_id('new_game')
        grid.click()
        time.sleep(2)
        ...
        try:
            self.browser.find_element_by_class_name('card_history_game')
